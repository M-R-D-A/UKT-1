            ...
            <div className='h-auto overflow-y-auto px-5 py-7 grid grid-flow-row auto-rows-max gap-y-6'>
                {events.map((item, index) => (
                    <button 
                        ...
                        onClick={() => router.push({pathname:`/siswa/dev/${router.query.tipe}/${router.query.ranting}/${encryptId(JSON.stringify({id:item.id_event,name:item.name}))}`, query: {events:item.name}})}
                    >
