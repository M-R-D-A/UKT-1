...
const belati = () => {
    ...
    const [datalistbelati, setdata] = useState()
    ...
    const getDatabelati = async () => {
        ...
        axios.get(BASE_URL + `belati/ukt/${dataSiswa.tipe_ukt}`, { headers: { Authorization: `Bearer ${token}` } })
            .then(res => {
                ...
                console.log(res)
                setDatabelati(res.data.data)
                const data = res.data.data
                for (let i = 0; i < res.data.limit; i++) {
                    const id_belati = data[i].id_belati
                    const selectedOption = null
                    updatedOptions.push({ id_belati, selectedOption })
