...
module.exports = {
    ...
    controllerGetByRantingFiltered: async (req, res) => {
        ...
        switch (jenis) {
            ...
            case 'all':
                orderCriteria.push([
                    Sequelize.literal('(COALESCE(senam, 0) + COALESCE(jurus, 0) + COALESCE(fisik, 0) + COALESCE(teknik, 0) + COALESCE(sambung, 0) + COALESCE(belati, 0) + COALESCE(kripen, 0) + COALESCE(senam_toya, 0) + COALESCE(jurus_toya, 0) + COALESCE(keshan, 0))/7'),
                    updown === 'downToUp' ? 'ASC' : 'DESC'
                ]);
                break;
            default:
